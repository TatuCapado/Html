...
class Admin
{
	...
	public function verificarPedidosRelacionados($idProduto)
	{
		$stmt = $this->con->prepare("SELECT COUNT(*) as total FROM pedidosvendas WHERE item_id = ?");
	...
	public function excluirPedidosRelacionados($idProduto)
	{
		$stmt = $this->con->prepare("DELETE FROM pedidosvendas WHERE item_id = ?");
		$stmt->bind_param("i", $idProduto);
		$stmt->execute();
	}
	public function deleteProduto($id)
	{
		if ($this->verificarPedidosRelacionados($id)) {
			// Se houver pedidos relacionados, exclua os pedidos primeiro
			$this->excluirPedidosRelacionados($id);
		}

		$stmt = $this->con->prepare("DELETE FROM " . $this->produtosTable . " WHERE id = ?");
